...
<task type="auto">
  ...
  <verify>
    ...
    - Custom SQL files include: metric_events (with PARTITION BY), RLS policies, Vault functions, org provisioning trigger
  ...
  <done>
    All database tables are defined in Drizzle schema files and generated as migrations. metric_events is partitioned by recorded_at with monthly partitions. RLS policies enforce org isolation via JWT claims on all tenant-scoped tables. Vault functions exist for encrypted credential storage. Org provisioning trigger auto-creates org + workspace + membership on new user signup.
  </done>
</task>
