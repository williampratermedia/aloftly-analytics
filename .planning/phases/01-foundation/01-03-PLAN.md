...
<task type="auto">
  ...
  <action>
    1. Create `src/middleware.ts` — session refresh + route protection (FOUN-02, FOUN-04):
       ...
       - Protected routes: anything under `/(dashboard)` or starting with `/dashboard`, `/onboarding`, `/settings`
    ...
    3. Create `src/app/(auth)/auth/callback/route.ts` — PKCE code exchange (FOUN-02):
       ...
       - On success: redirect to `next` param or `/onboarding` (default for new users)
